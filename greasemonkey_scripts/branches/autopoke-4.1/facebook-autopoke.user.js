...
function poke_function(poke_link) { 
     ...
     var post_data_array = new Array('uid=' + poke_uid, 'pokeback=1', '__d=1', 'post_form_id=' + _fbconfig.post_form_id, 
                         'fb_dtsg=' + _fbconfig.fb_dtsg, 'lsd=', 'post_form_id_source=AsyncRequest', '__user=' + _fbconfig.user_uid, '__a=1');

     var post_data = post_data_array.join('&');
...
function execute_poke(xml) {
     var postURI = '/ajax/poke.php?__a=1';
     var poke_uid = evaluate_xpath('.//input[@name="uid"]', xml).snapshotItem(0).getAttribute('value');

     var post_data_array = new Array('__d=1', '__user=' + _fbconfig.user_uid, 'fb_dtsg=' + _fbconfig.fb_dtsg,
                         'lsd=', 'nctr[_mod]=pagelet_pokes', 'post_form_id_source=AsyncRequest' );

     /* As of version 4.1, facebook provides the following variables for
        poke target:
          * post_form_id
          * uid
          * pokeback
          * opp (no value)
          * pk01 (language-specific "Poke" button) */
     ...
     for (var i = 0; i < input_nodes.snapshotLength; i++) {
          if (input_nodes.snapshotItem(i).hasAttribute('value'))
               post_data_array.push(input_nodes.snapshotItem(i).getAttribute('name') + "=" + input_nodes.snapshotItem(i).getAttribute('value'));
          else 
               post_data_array.push(input_nodes.snapshotItem(i).getAttribute('name') + "=");
     }
...
function fbconfig_object() {
     var _f = new Object;

     _f.fb_dtsg = get_var_value('fb_dtsg');
     _f.post_form_id = get_var_value('post_form_id');
     _f.user_uid = 0;
